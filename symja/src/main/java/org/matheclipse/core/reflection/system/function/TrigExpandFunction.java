...
public class TrigExpandFunction implements Function<IExpr, IExpr> {
    /**
    ...
    private static IExpr expandTanhPlus(IAST plusAST, int startPosition) {
        ...
        if (startPosition == plusAST.size() - 2) {
            ...
            result.append(Power(Plus(F.C1, Times(F.Tanh(lhs), F.Tanh(rhs))), CN1));
        } else {
            result.append(Plus(F.Tanh(lhs), expandTanhPlus(plusAST, startPosition + 1)));
            result.append(
                    Power(Plus(F.C1, Times(F.Tanh(lhs), expandTanhPlus(plusAST, startPosition + 1))), CN1));
        }
        return result;
    }
